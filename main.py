...
# Placeholder functions for modular SEO checks
def run_title_check(url):
    """Placeholder for title tag analysis"""
    return {
        'status': 'pass',
        'score': 85,
        'message': 'Title tag is well optimized',
        'details': f'Title length: 45 characters, includes primary keyword'
    }

def run_meta_check(url):
    """Placeholder for meta description analysis"""
    return {
        'status': 'warning',
        'score': 70,
        'message': 'Meta description could be improved',
        'details': f'Meta description length: 145 characters, compelling but could include more keywords'
    }

def run_headings_check(url):
    """Placeholder for heading structure analysis"""
    return {
        'status': 'pass',
        'score': 90,
        'message': 'Heading structure is well organized',
        'details': f'1 H1 tag, 4 H2 tags, 7 H3 tags found'
    }

def run_images_check(url):
    """Placeholder for image optimization analysis"""
    return {
        'status': 'fail',
        'score': 45,
        'message': '8 images missing alt text',
        'details': f'Total images: 15, Missing alt text: 8, Optimized: 7'
    }

def run_content_check(url):
    """Placeholder for content quality analysis"""
    return {
        'status': 'pass',
        'score': 80,
        'message': 'Content quality is good',
        'details': f'Word count: 1,245, Readability score: 68, Keyword density: 2.3%'
    }

def run_ssl_check(url):
    """Placeholder for SSL certificate analysis"""
    return {
        'status': 'pass',
        'score': 100,
        'message': 'SSL certificate is valid and secure',
        'details': f'HTTPS enabled, valid certificate, strong encryption'
    }

def run_performance_check(url):
    """Placeholder for Core Web Vitals analysis"""
    return {
        'status': 'warning',
        'score': 65,
        'message': 'Performance needs improvement',
        'details': f'LCP: 2.8s, FID: 85ms, CLS: 0.12 - Mobile performance score: 65'
    }

def run_sitemap_check(url):
    """Placeholder for sitemap analysis"""
    return {
        'status': 'fail',
        'score': 0,
        'message': 'XML sitemap not found',
        'details': f'No sitemap.xml found at standard locations'
    }

def run_robots_check(url):
    """Placeholder for robots.txt analysis"""
    return {
        'status': 'pass',
        'score': 95,
        'message': 'Robots.txt is properly configured',
        'details': f'robots.txt found, correctly formatted, allows crawling'
    }

def run_mobile_check(url):
    """Placeholder for mobile friendliness analysis"""
    return {
        'status': 'pass',
        'score': 88,
        'message': 'Mobile-friendly design detected',
        'details': f'Responsive design, mobile viewport set, touch-friendly elements'
    }

def run_broken_links_check(url):
    """Placeholder for broken links analysis"""
    return {
        'status': 'warning',
        'score': 75,
        'message': '3 broken links found',
        'details': f'Total links checked: 127, Broken: 3, Working: 124'
    }

def run_internal_links_check(url):
    """Placeholder for internal linking analysis"""
    return {
        'status': 'pass',
        'score': 82,
        'message': 'Good internal linking structure',
        'details': f'Average internal links per page: 12, Well distributed'
    }

def run_orphan_pages_check(url):
    """Placeholder for orphan pages analysis"""
    return {
        'status': 'warning',
        'score': 70,
        'message': '2 orphan pages detected',
        'details': f'Sitemap pages: 45, Linked pages: 43, Orphaned: 2'
    }

def run_redirects_check(url):
    """Placeholder for redirect chains analysis"""
    return {
        'status': 'pass',
        'score': 95,
        'message': 'No redirect chains detected',
        'details': f'All redirects are direct (301/302), no chains or loops'
    }

def run_navigation_check(url):
    """Placeholder for navigation structure analysis"""
    return {
        'status': 'pass',
        'score': 85,
        'message': 'Navigation structure is user-friendly',
        'details': f'Clear menu, breadcrumbs present, logical hierarchy'
    }

def run_design_check(url):
    """Placeholder for design consistency analysis"""
    return {
        'status': 'pass',
        'score': 90,
        'message': 'Design is consistent across pages',
        'details': f'Uniform color scheme, consistent typography, aligned layouts'
    }
...
def run_accessibility_check(url):
    ...
    return {
        ...
        'details': f'ARIA labels present, color contrast good, keyboard navigation needs work'
    }

def run_conversion_check(url):
    """Placeholder for conversion elements analysis"""
    return {
        'status': 'pass',
        'score': 78,
        'message': 'Good conversion optimization',
        'details': f'CTA above fold, contact info visible, trust signals present'
    }

def run_link_profile_check(url):
    """Placeholder for link profile analysis"""
    return {
        'status': 'pass',
        'score': 75,
        'message': 'Healthy backlink profile',
        'details': f'Total backlinks: 1,284, Domain rating: 54, DoFollow ratio: 76%'
    }

def run_anchor_text_check(url):
    """Placeholder for anchor text analysis"""
    return {
        'status': 'pass',
        'score': 82,
        'message': 'Good anchor text distribution',
        'details': f'Branded: 45%, Generic: 28%, Exact match: 13% - Well balanced'
    }

def run_toxic_links_check(url):
    """Placeholder for toxic links analysis"""
    return {
        'status': 'warning',
        'score': 85,
        'message': '7 potentially toxic links detected',
        'details': f'High spam score domains: 7, Recommend disavow review'
    }

def run_referring_domains_check(url):
    """Placeholder for referring domains analysis"""
    return {
        'status': 'pass',
        'score': 80,
        'message': 'Quality referring domains',
        'details': f'Unique domains: 432, High authority: 98, Average DR: 54'
    }

# Modular check runner
def run_modular_seo_checks(url, selected_checks):
    """Run only the selected SEO checks and return results"""
    results = {}
    
    # Map check names to functions
    check_functions = {
        'on_page': {
            'titles': run_title_check,
            'meta': run_meta_check,
            'headings': run_headings_check,
            'images': run_images_check,
            'content': run_content_check,
        },
        'technical': {
            'ssl': run_ssl_check,
            'performance': run_performance_check,
            'sitemap': run_sitemap_check,
            'robots': run_robots_check,
            'mobile': run_mobile_check,
        },
        'link_analysis': {
            'broken_links': run_broken_links_check,
            'internal_links': run_internal_links_check,
            'orphan_pages': run_orphan_pages_check,
            'redirects': run_redirects_check,
        },
        'uiux': {
            'navigation': run_navigation_check,
            'design': run_design_check,
            'accessibility': run_accessibility_check,
            'conversion': run_conversion_check,
        },
        'backlink': {
            'link_profile': run_link_profile_check,
            'anchor_text': run_anchor_text_check,
            'toxic_links': run_toxic_links_check,
            'referring_domains': run_referring_domains_check,
        }
    }
    
    # Run selected checks
    for category, checks in selected_checks.items():
        if category in check_functions:
            results[category] = {}
            for check in checks:
                if check in check_functions[category]:
                    try:
                        logger.info(f"Running {category}.{check} check for {url}")
                        results[category][check] = check_functions[category][check](url)
                    except Exception as e:
                        logger.error(f"Error running {category}.{check} check: {e}")
                        results[category][check] = {
                            'status': 'error',
                            'score': 0,
                            'message': f'Check failed: {str(e)}',
                            'details': 'Unable to complete this check'
                        }
    
    return results

@app.route('/generate-modular-pdf', methods=['POST'])
def generate_modular_pdf():
    """Generate a modular SEO audit PDF with only selected checks"""
    try:
        ...
        logger.info(f"Starting modular audit for: {url}")
        logger.info(f"Selected checks: {selected_checks}")

        # Validate that at least one check is selected
        total_selected = sum(len(checks) for checks in selected_checks.values())
        if total_selected == 0:
            return jsonify({'error': 'No checks selected. Please select at least one SEO check.'}), 400

        # Run selected checks
        check_results = run_modular_seo_checks(url, selected_checks)
        
        # Generate filename
        domain = urllib.parse.urlparse(url).netloc
        clean_domain = domain.replace('www.', '')
        domain_for_filename = re.sub(r'[^\w\-_]', '_', clean_domain)
        filename = f"seo_audit_modular_{domain_for_filename}.pdf"
        ...
        # Generate modular PDF report
        result = generate_modular_pdf_report(check_results, selected_checks, url, filepath)
        
        if result is None:
            logger.error("Modular PDF generation failed")
            return jsonify({'error': 'Failed to generate modular PDF report'}), 500

        # Verify file exists and has content
        if not os.path.exists(filepath):
            logger.error(f"Modular PDF file not found: {filepath}")
            return jsonify({'error': 'Report file not found after generation'}), 500

        file_size = os.path.getsize(filepath)
        if file_size == 0:
            logger.error(f"Modular PDF file is empty: {filepath}")
            return jsonify({'error': 'Generated report file is empty'}), 500

        logger.info(f"Generated modular report: {filename} ({file_size} bytes)")
        ...
        logger.error(f"Error generating modular PDF: {e}")
        return jsonify({'error': str(e)}), 500

def generate_modular_pdf_report(check_results, selected_checks, url, filename):
    """Generate a modular PDF report with only selected sections"""
    try:
        doc = SimpleDocTemplate(filename, pagesize=A4)
        story = []
        ...
        # Title page
        story.append(Paragraph("Modular SEO Audit Report", title_style))
        story.append(Spacer(1, 20))
        
        # Website info
        domain = urllib.parse.urlparse(url).netloc
        story.append(Paragraph(f"<b>Website:</b> {domain}", body_style))
        story.append(Paragraph(f"<b>Report Date:</b> {datetime.now().strftime('%Y-%m-%d %H:%M')}", body_style))
        story.append(Paragraph(f"<b>Audit Type:</b> Custom Modular Analysis", body_style))
        story.append(Spacer(1, 30))

        # Summary of selected checks
        story.append(Paragraph("Selected SEO Checks", heading_style))
        for category, checks in selected_checks.items():
            if checks:
                category_name = category.replace('_', ' ').title()
                story.append(Paragraph(f"<b>{category_name}:</b> {', '.join(checks)}", body_style))
        story.append(Spacer(1, 30))
        ...
        for category_results in check_results.values():
            for check_result in category_results.values():
                total_score += check_result.get('score', 0)
                total_checks += 1
        
        overall_score = int(total_score / total_checks) if total_checks > 0 else 0
        
        # Overall score display
        story.append(Paragraph("Overall SEO Score", heading_style))
        score_color = HexColor('#4CAF50') if overall_score >= 80 else HexColor('#FF9800') if overall_score >= 60 else HexColor('#F44336')
        
        score_table = Table([[f"{overall_score}/100"]], colWidths=[2*inch])
        ...
        category_names = {
            'on_page': '📄 On-Page SEO Results',
            'technical': '⚙️ Technical SEO Results',
            'link_analysis': '🔗 Link Analysis Results',
            'uiux': '🎨 UI/UX Audit Results',
            'backlink': '🔗 Backlink Audit Results'
        }

        for category, results in check_results.items():
            if results:  # Only show categories that have results
                story.append(PageBreak())
                story.append(Paragraph(category_names.get(category, category.title()), heading_style))
                story.append(Spacer(1, 20))
                
                # Create results table for this category
                table_data = [['Check', 'Score', 'Status', 'Details']]
                ...
                results_table = Table(table_data, colWidths=[1.5*inch, 0.8*inch, 0.8*inch, 3.2*inch])
                table_style = [
                    ('BACKGROUND', (0, 0), (-1, 0), HexColor('#A23B72')),
                    ('TEXTCOLOR', (0, 0), (-1, 0), white),
                    ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
                ...
                # Color code scores and status
                for i in range(1, len(table_data)):
                    # Color code score
                    try:
                        score_text = table_data[i][1]
                    ...
                    # Color code status
                    status_text = table_data[i][2].lower()
                    if status_text == 'pass':
                        status_color = HexColor('#4CAF50')  # Green
                        ...
                        status_color = HexColor('#9E9E9E')  # Gray
                    
                    table_style.append(('BACKGROUND', (2, i), (2, i), status_color))
                    table_style.append(('TEXTCOLOR', (2, i), (2, i), white))
                    table_style.append(('FONTNAME', (2, i), (2, i), 'Helvetica-Bold'))
        ...
        for category, results in check_results.items():
            for check_name, check_result in results.items():
                ...
                if status == 'fail' or score < 50:
                    priority_issues.append(f"• {check_name.replace('_', ' ').title()}: {check_result.get('message', 'Needs attention')}")
                elif status == 'warning' or score < 70:
                    recommendations.append(f"• {check_name.replace('_', ' ').title()}: {check_result.get('message', 'Could be improved')}")

        if priority_issues:
            story.append(Paragraph("🚨 Priority Issues (Immediate Action Required):", body_style))
            for issue in priority_issues[:5]:  # Show top 5 priority issues
                story.append(Paragraph(issue, body_style))
            story.append(Spacer(1, 15))

        if recommendations:
            story.append(Paragraph("⚠️ Improvement Opportunities:", body_style))
            for rec in recommendations[:8]:  # Show top 8 recommendations
                story.append(Paragraph(rec, body_style))
            story.append(Spacer(1, 15))

        if not priority_issues and not recommendations:
            story.append(Paragraph("✅ Excellent! All selected SEO checks passed with good scores.", body_style))

        # Build PDF
        doc.build(story)
        logger.info(f"Modular PDF report generated successfully: {filename}")
        return filename

    except Exception as e:
        logger.error(f"Error generating modular PDF report: {e}")
        return None
